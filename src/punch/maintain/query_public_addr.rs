...
pub(crate) async fn query_tcp_public_addr_loop(
    ...
    let tcp_stun_servers = punch_context.tcp_stun_servers.clone();
            ...
            match stun.to_socket_addrs() {
                Ok(mut addr) => {
                    if let Some(addr) = addr.next() {
                        if let Some(w) = socket_manager.tcp_socket_manager_as_ref() {
                    ...
                    if let Err(e) = rs {
                        ...
                        match stun_tcp_read(&mut tcp_stream).await {
                            Ok(addr) => {
                                ...
                                punch_context.update_tcp_public_addr(addr);
                                let mut buf = [0; 1024];
                                loop {
...
pub(crate) async fn query_udp_public_addr_loop(
    ...
    let udp_stun_servers = punch_context.udp_stun_servers.clone();
